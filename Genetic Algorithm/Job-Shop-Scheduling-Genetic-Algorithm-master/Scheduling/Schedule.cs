...
namespace Scheduling
{
    public class Schedule
    {
        ...
        public void DrawSchedule(Graphics grp, Control panel)
        {
            ...
            string[,] finishTimeToTxt;
            int job_n = 100;
            int mac_n = 10;
            finishTimeToTxt = new string[job_n, mac_n];
            
            for (int i = 0; i < job_n; i++)
            {
                for (int j = 0; j < mac_n; j++)
                {
                    finishTimeToTxt[i, j] = finishTimes[i, j].ToString();
                }
                
            }

            using (StreamWriter outputFile = new StreamWriter(@"G:\내 드라이브\Job-Shop-Scheduling-Genetic-Algorithm-master\finishtime_j100_dat100.txt"))
            {
                for (int i = 0;i< job_n; i++)
                { 
                    for(int j = 0;j< mac_n; j++)
                    {
                        outputFile.Write(finishTimeToTxt[i, j].ToString());
                        outputFile.Write(" ");
                    }
                    outputFile.Write("\n");
