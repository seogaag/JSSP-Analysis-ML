...
def final_evaluation(config: dict, data_test: List[List[Task]], logger: Logger):
    ...
    for tasks in data_test:
        list_tasks = []
        for task in tasks:
            list_tasks.append(task.runtime)
            if len(list_tasks) == 10:
                list_runtime.append(list_tasks)
                list_tasks = []
        
        if(len(list_runtime)) == 100:
            print("문제")
            print(list_runtime)
            list_runtime = []
            i += 1
...
def main(config_file_name: dict = None, external_config: dict = None) -> None:
    ...
    train_data, test_data = train_test_split(
        data, train_size=config.get('train_test_split'), shuffle=False, random_state=split_random_seed)
    test_data, val_data = train_test_split(
        test_data, train_size=config.get('test_validation_split'), shuffle=False, random_state=split_random_seed)
