...
def test_model_and_heuristic(config: dict, model, data_test: List[List[Task]], logger: Logger,
    ...
    data_path = "C:\\Users\\wlsdm\\Desktop\\진은서\\대학교\\4학년\\시스템종합설계\\schlably-main\\Result_j50_ppo.txt"
 
    with open(data_path,'w') as file:
            
        # makespan값과 tardiness값을 str형태로 저장한 list구현
        list_makespan = []
        for i in res.get('makespan'):
            list_makespan.append(str(i))
        list_tardiness = []
        for i in res.get('tardiness'):
            list_tardiness.append(str(i))  
        list_execution_time = []
        for i, data in enumerate(data_test):
            start_time = time.time()
            test_data = [data]  
            test_kwargs['data'] = test_data
            test_model(model=model, **test_kwargs)
            end_time = time.time()
        ...
        list_all = []
        for i in range(len(list_makespan)):
            arr = []
            dat_n = 100
            arr.append(dat_n+i)
            arr.append(list_makespan[i])
            arr.append(list_tardiness[i])
            arr.append(list_execution_time[i])
            list_all.append(arr)

        for i in list_all:
            file.writelines(["data: ",str(i[0]), "\n makespan = ",str(i[1]) , "\n tardiness = ", str(i[2]),"\n Time = {} seconds".format(i[3]),"\n\n"])
